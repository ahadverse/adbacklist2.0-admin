...
const TinyMce = () => {
  ...
  const fetchDetails = async () => {
    try {
      ...
      setDetails(res.data?.response?.token);
      setForm(res.data?.response?.token);
      setLoading(false);
    } catch (err) {
      setLoading(false);
      console.error(err);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { _id, ...rest } = form;
      await baseApi.put(`/tiny-mce/${_id}`, rest);
      setReload(!reload);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white text-black dark:text-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
        ...
        <h1 className="text-2xl font-bold mb-6">Tiny MCE Token</h1>


      {/* Details */}
      <div className="space-y-2 bg-gray-50 dark:bg-gray-900 p-4 rounded text-center">
      
        <p className="break-all">
          <span className="font-bold text-red-700">Token:</span> {details.token}
        </p>
      </div>
