...
const ResponsiveLinks = () => {
  ...
  const fetchDetails = async () => {
    try {
      const res = await baseApi.get("/responsive-ads");
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { _id, ...rest } = form;
      await baseApi.put(`/responsive-ads/${_id}`, rest);
      setReload(!reload);
  ...
  return (
    ...
    <div className=" flex justify-between items-center">
        <h1 className="text-2xl font-bold mb-6">Responsive Links</h1>
 <UploadImageModal />
    </div>
      {/* Details */}
      <div className="space-y-2 bg-gray-50 dark:bg-gray-900 p-4 rounded text-center">
        ...
        <p className="break-all">
          <span className="font-bold text-red-700">Link:</span> {details.link}
        </p>
      </div>
